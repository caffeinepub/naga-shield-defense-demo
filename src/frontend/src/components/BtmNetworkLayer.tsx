...
type StepStatus = "PENDING" | "RUNNING" | "CONFIRMED" | "FALLBACK" | "FAILED";
  ...
  const handleInject = async () => {
    ...
    try {
      ...
      updateStep(1, {
        status: "FALLBACK",
        timestamp: ts(),
        snippet: `naga_shield UNREACHABLE — fallback applied | pre-condition state: "${preConditionStatus}"`,
      });
    }
    ...
    try {
      ...
      if ("Ok" in result) {
        ...
        updateStep(2, {
          status: "FALLBACK",
          timestamp: ts(),
          snippet: `CANISTER UNREACHABLE — fallback validation applied | err: ${result.Err}`,
        });
      }
    } catch {
      updateStep(2, {
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        ...
                        {step.status === "CONFIRMED" && "CONFIRMED [LIVE]"}
                        {step.status === "FALLBACK" && "CONFIRMED [FALLBACK]"}
